...
namespace JCSV.Config
{
    ...
    public class TypeInferringDataTableConverter : Newtonsoft.Json.Converters.DataTableConverter
    {
        ...
        public TypeInferringDataTableConverter()
        {

        }

        public TypeInferringDataTableConverter(ConverterOptions options)
        {
            _options = options;
        }
        ...
        public override void WriteJson(JsonWriter writer, object? value, JsonSerializer serializer)
        {
            ...
            foreach (DataRow row in table.Rows)
            {
                ...
                foreach (DataColumn column in row.Table.Columns)
                {
                    ...
                    if (_options.TreatEmptyStringAsNull)
                    {
                    ...
                    if (_options.PreserveUnderlyingTypes)
                    {
                        if (int.TryParse((string)columnValue, out var valueInt))
                        {
                            serializer.Serialize(writer, valueInt);
                        }
                        else if (bool.TryParse((string)columnValue, out var valueBool))
                        {
                            serializer.Serialize(writer, valueBool);
                        }
                        else
                        {
